...
var PhoneScheme = PhoneScheme || {};

PhoneScheme.Version = "0.7.0";  // TODO: deprecate this
PhoneScheme.VERSION = "0.7.0";
PhoneScheme.GitCommit = "";
...
PhoneScheme.nil = {
  ...
  to_set: function() { return new PhoneScheme.Set(); },
...
//
PhoneScheme.undef = new Object();
PhoneScheme.undef.toString = function(){ return "#<undef>"; }

// Prints the arguments to console.debug.
PhoneScheme.debug = function(/*arguments*/){
  var args = _.toArray(arguments);
  console.debug.apply(console, _.map(args, PhoneScheme.inspect));
}

//
// Assertion
//
PhoneScheme.assert = function(cond, desc) {
  if (!cond) {
    throw new PhoneScheme.Bug("[BUG] Assertion failed: "+desc);
  }
}
...
PhoneScheme.max_trace_size = 40;

// Stop showing deprecation warning
PhoneScheme.suppress_deprecation_warning = false;
//
// Super-simple class implementation
//
// Example usage:
//
// PhoneScheme.Foo = PhoneScheme.Class.create({
...
// });
//
// PhoneScheme.Bar = PhoneScheme.Class.extend(new PhoneScheme.Foo("hello1"), {
...
//
// - klass : a class defined by PhoneScheme.Class.create
// - name_or_names : method name (a string or an array of strings)
//
// Example
//
//   // Given this method
//   PhoneScheme.Enumeration.EnumType = ...
//     universe: function(){
//       return ...
//     }
//   ...
//   // Memoize
//   PhoneScheme.Class.memoize(PhoneScheme.Enumeration.EnumType,
//                            "_universe"); 
//
//   // Equal to:
//   PhoneScheme.Enumeration.EnumType = ...
...
//     }
//   ...
PhoneScheme.Class.memoize = function(klass, name_or_names){
...
PhoneScheme.to_write = function(obj){
  if(obj === undefined)
    return "undefined";
  else if(obj === null)
    return "null";
    ...
    return '"' +
           obj.replace(/\\|\"/g,function($0){return'\\'+$0;})
              ...
              .replace(/\r/g, "\\r") +
           '"';
  else if(PhoneScheme.isClosure(obj))
    return "#<Closure>";
  else if(_.isArray(obj))
    return "#(" + _.map(obj, function(e) { return PhoneScheme.to_write(e); }).join(" ") + ")";
  ...
  return PhoneScheme.inspect(obj);
}
...
PhoneScheme.to_display = function(obj){
  if(_.isUndefined(obj))
    ...
    return obj;
  else if(obj instanceof PhoneScheme.Symbol)
    return obj.name;
  else if(obj instanceof Array)
    return '#(' + _.map(obj, PhoneScheme.to_display).join(' ') + ')';
  else if(obj instanceof PhoneScheme.Pair)
    return obj.inspect(PhoneScheme.to_display);
  else if(obj instanceof PhoneScheme.Char)
    return obj.value;
  else
    return PhoneScheme.to_write(obj);
}
...
//TODO: support Values
PhoneScheme.write_ss = function(obj, array_mode){
  var known = [obj], used = [false];
  PhoneScheme.find_cyclic(obj, known, used);
  var cyclic   = PhoneScheme.reduce_cyclic_info(known, used);
  var appeared = new Array(cyclic.length);
  for(var i=cyclic.length-1; i>=0; i--) appeared[i] = false;

  return PhoneScheme.to_write_ss(obj, cyclic, appeared, array_mode);
}
PhoneScheme.to_write_ss = function(obj, cyclic, appeared, array_mode){
  var ret = "";
  ...
  if(obj instanceof PhoneScheme.Pair){
    var a = [];
    a.push(PhoneScheme.to_write_ss(obj.car, cyclic, appeared, array_mode));
    for(var o=obj.cdr; o != PhoneScheme.nil; o=o.cdr){
      if(!(o instanceof PhoneScheme.Pair) || cyclic.indexOf(o) >= 0){
        a.push(".");
        a.push(PhoneScheme.to_write_ss(o, cyclic, appeared, array_mode));
        break;
      }
      a.push(PhoneScheme.to_write_ss(o.car, cyclic, appeared, array_mode));
    }
    ret += "(" + a.join(" ") + ")";
  }
  else if(obj instanceof Array){
    var a = _.map(obj, function(item){
      return PhoneScheme.to_write_ss(item, cyclic, appeared, array_mode);
    })
    if(array_mode)
      ret += "[" + a.join(", ") + "]";
    else
    ...
    ret += PhoneScheme.to_write(obj);
  }
  return ret;
}
PhoneScheme.reduce_cyclic_info = function(known, used){
  var n_used = 0;
...
PhoneScheme.find_cyclic = function(obj, known, used){
  var items = (obj instanceof PhoneScheme.Pair)  ? [obj.car, obj.cdr] :
  ...
  _.each(items, function(item){
    if(typeof(item)=='number' || typeof(item)=='string' ||
      item === PhoneScheme.undef || item === true || item === false ||
      item === PhoneScheme.nil || item instanceof PhoneScheme.Symbol) return;
    ...
    if(i >= 0)
      ...
      known.push(item);
      used.push(false);
      PhoneScheme.find_cyclic(item, known, used);
    }
  });
};
...
// inspect
//
PhoneScheme.inspect = function(object, opts) {
  try {
    ...
    if (_.isArray(object)) {
      return '[' + _.map(object, PhoneScheme.inspect).join(', ') + ']';
    }
...
PhoneScheme.isNil = function(obj){
  return (obj === PhoneScheme.nil);
};

PhoneScheme.isUndef = function(obj){
  return (obj === PhoneScheme.undef);
};

PhoneScheme.isBoolean = _.isBoolean; // Return true if arg is either true or false

//PhoneScheme.isNumber is defined in number.js (Return true if arg is scheme number)

PhoneScheme.isString = _.isString;

PhoneScheme.isChar = function(obj){
  return (obj instanceof PhoneScheme.Char);
};

PhoneScheme.isSymbol = function(obj){
  return (obj instanceof PhoneScheme.Symbol);
};

PhoneScheme.isPort = function(obj){
  return (obj instanceof PhoneScheme.Port);
};

// Note: '() is not a pair in scheme
PhoneScheme.isPair = function(obj){
  return (obj instanceof PhoneScheme.Pair);
};

// Returns true if obj is a proper list
// Note: isList returns true for '()
PhoneScheme.isList = function(obj){
  var nil = PhoneScheme.nil, Pair = PhoneScheme.Pair;
...
PhoneScheme.isVector = function(obj){
  return (obj instanceof Array) && (obj.closure_p !== true);
};

PhoneScheme.isHashtable = function(obj){
  return (obj instanceof PhoneScheme.Hashtable);
};

PhoneScheme.isMutableHashtable = function(obj){
  return (obj instanceof PhoneScheme.Hashtable) && obj.mutable;
};

// Returns true if `obj` is a Scheme closure.
PhoneScheme.isClosure = function(obj){
  return (obj instanceof Array) && (obj.closure_p === true);
};

// Change `ary` into a Scheme closure (destructive).
PhoneScheme.makeClosure = function(ary) {
  ary.closure_p = true;
  return ary;
};
...
PhoneScheme.isProcedure = function(obj){
  return PhoneScheme.isClosure(obj) || _.isFunction(obj);
};

// Return true if obj is a scheme value which evaluates to itself
PhoneScheme.isSelfEvaluating = function(obj) {
  return PhoneScheme.isBoolean(obj) ||
         PhoneScheme.isNumber(obj) ||
         PhoneScheme.isString(obj) ||
         PhoneScheme.isChar(obj);
};

//
// equality
//
PhoneScheme.eq = function(a, b){
  return a === b;
};
// TODO: Records (etc.)
PhoneScheme.eqv = function(a, b){
  return a == b && (typeof(a) == typeof(b));
};
PhoneScheme.equal = function(a, b){
  //TODO: must terminate for cyclic objects
  return PhoneScheme.to_write(a) == PhoneScheme.to_write(b);
};
...
PhoneScheme.Error = PhoneScheme.Class.create({
...
PhoneScheme.Bug = PhoneScheme.Class.extend(new PhoneScheme.Error(), {
  initialize: function(msg){
...
PhoneScheme.UserError = PhoneScheme.Class.extend(new PhoneScheme.Error(), {
  initialize: function(msg){
    this.message = msg;
  }
});
...
//
PhoneScheme.Set = PhoneScheme.Class.create({
  ...
  set_cons : function(item){
    var o = new PhoneScheme.Set(item);
    o.arr = _.clone(this.arr);
    o.arr.push(item);
    return o;
  },
  set_union : function(/*args*/){
    var o = new PhoneScheme.Set();
    o.arr = _.clone(this.arr);

    for(var k=0; k<arguments.length; k++){
      var s2 = arguments[k];
      if(!(s2 instanceof PhoneScheme.Set))
        throw new PhoneScheme.Error("set_union: arguments must be a set");
  ...
  set_intersect : function(s2){
    if(!(s2 instanceof PhoneScheme.Set))
      throw new PhoneScheme.Error("set_intersect: arguments must be a set");
  ...
  set_minus : function(s2){
    if(!(s2 instanceof PhoneScheme.Set))
      throw new PhoneScheme.Error("set_minus: arguments must be a set");
...
// Values
//
PhoneScheme.Values = PhoneScheme.Class.create({
  initialize: function(values){
    this.content = values;
  },
  to_write: function(){
    return "#<Values " +
             _.map(this.content, PhoneScheme.to_write).join(" ") +
           ">";
  }
});
...
PhoneScheme.Pair = PhoneScheme.Class.create({
  ...
  to_array: function(){
    var ary = [];
    for(var o = this; o instanceof PhoneScheme.Pair; o=o.cdr){
      ary.push(o.car);
    }
    return ary;
  },
  ...
  last_cdr: function(){
    var o;
    for(o = this; o instanceof PhoneScheme.Pair; o = o.cdr)
      ;
    return o;
  },
  ...
  map: function(func){
    var ary = [];
    for(var o = this; PhoneScheme.isPair(o); o = o.cdr){
      ary.push(func(o.car));
    }
    return ary;
  },
  ...
  concat: function(list){
    var o = this;
    while(o instanceof PhoneScheme.Pair && o.cdr != PhoneScheme.nil){
      o = o.cdr;
    }
    o.cdr = list;
    return this;
  ...
  inspect: function(conv){
    conv || (conv = PhoneScheme.inspect);
    var a = [];
    var last = this.foreach(function(o){
      a.push(conv(o));
    });
    if(last != PhoneScheme.nil){
      a.push(".");
      a.push(conv(last));
    }
    return "(" + a.join(" ") + ")";
  ...
  to_write: function(){
    return this.inspect(PhoneScheme.to_write);
  }
});
...
// Example:
//   PhoneScheme.List(1, 2, [3, 4]) ;=> (list 1 2 (vector 3 4))
//   PhoneScheme.deep_array_to_list(1, 2, [3, 4]) ;=> (list 1 2 (list 3 4))
var array_to_list = function(ary, deep) {
  var list = PhoneScheme.nil;
  for(var i=ary.length-1; i>=0; i--){
    var obj = ary[i];
    if(deep && _.isArray(obj) && !obj.is_vector){
      obj = array_to_list(obj, deep);
    }
    list = new PhoneScheme.Pair(obj, list);
  }
  return list;
}

// Shallow: List(1, 2, [3]) == (list 1 2 (vector 3 4))
PhoneScheme.List = function() {
  var ary = _.toArray(arguments);
  return array_to_list(ary, false);
};

// Shallow: array_to_list(1, 2, [3]) == (list 1 2 (vector 3 4))
PhoneScheme.array_to_list = function(ary) {
  return array_to_list(ary, false);
};

// Deep: deep_array_to_list(1, 2, [3, 4]) == (list 1 2 (list 3 4))
// deep_array_to_list([1, 2, 3]) - deep
PhoneScheme.deep_array_to_list = function(ary) {
  return array_to_list(ary, true);
};

PhoneScheme.Cons = function(car, cdr) {
  return new PhoneScheme.Pair(car, cdr);
};
//
// Symbol
//

PhoneScheme.Symbol = PhoneScheme.Class.create({
  initialize: function(str){
    this.name = str;
    PhoneScheme.Symbols[ str ] = this;
  },
...
PhoneScheme.Symbols = {};
PhoneScheme.Sym = function(name,leaveCase){
  if( PhoneScheme.Symbols[name] === undefined ){
    return new PhoneScheme.Symbol(name);
  }
  else if( ! (PhoneScheme.Symbols[name] instanceof PhoneScheme.Symbol) ){ //pre-defined member (like 'eval' in Firefox)
    return new PhoneScheme.Symbol(name);
  }
  else{
    return PhoneScheme.Symbols[name];
  }
}

PhoneScheme.gensym = function(){
  return PhoneScheme.Sym(_.uniqueId("__gensym"));
};
//
// Char
//

PhoneScheme.Char = PhoneScheme.Class.create({
  initialize: function(c){
    PhoneScheme.Chars[ this.value = c ] = this;
  },
  to_write: function(){
    switch(this.value){
      case '\n': return "#\\newline";
...
PhoneScheme.Chars = {};
PhoneScheme.Char.get = function(c) {
  if(typeof(c) != "string") {
    throw new PhoneScheme.Bug("Char.get: " +
                             PhoneScheme.inspect(c) + " is not a string");
  }
  if( PhoneScheme.Chars[c] === undefined )
    return new PhoneScheme.Char(c);
  else
    return PhoneScheme.Chars[c];
};
...
// Complex
//
PhoneScheme.Complex = PhoneScheme.Class.create({
  ...
  isRational: function() {
    return this.imag == 0 && PhoneScheme.isRational(this.real);
  },
  isInteger: function(){
    return this.imag == 0 && PhoneScheme.isInteger(this.real);
  },
...
PhoneScheme.Complex.from_polar = function(r, theta){
  var real = r * Math.cos(theta);
  var imag = r * Math.sin(theta);
  return new PhoneScheme.Complex(real, imag);
}
PhoneScheme.Complex.assure = function(num){
  if(num instanceof PhoneScheme.Complex)
    return num
  else
    return new PhoneScheme.Complex(num, 0);
}

//
// Rational (unfinished)
//
PhoneScheme.Rational = PhoneScheme.Class.create({
...
// Predicates
//
PhoneScheme.isNumber = function(x) {
  return (x instanceof PhoneScheme.Complex)  ||
         (x instanceof PhoneScheme.Rational) ||
         (typeof(x) == 'number');
};
PhoneScheme.isComplex = PhoneScheme.isNumber;
PhoneScheme.isReal = function(x) {
  if (x instanceof PhoneScheme.Complex || x instanceof PhoneScheme.Rational) {
    return x.isReal()
  }
  else {
...
PhoneScheme.isRational = function(x) {
  if (x instanceof PhoneScheme.Complex) {
    return x.isRational();
  }
  else if (x instanceof PhoneScheme.Rational) {
    return true;
  }
  else {
    return (typeof(x) == 'number');
...
PhoneScheme.isInteger = function(x) {
  if (x instanceof PhoneScheme.Complex || x instanceof PhoneScheme.Rational) {
    return x.isInteger();
  }
  else {
...
// (eof-object)
PhoneScheme.eof = new Object;

PhoneScheme.Port = PhoneScheme.Class.create({
...
// string ports (srfi-6)
//
PhoneScheme.Port.StringOutput = PhoneScheme.Class.extend(new PhoneScheme.Port(false, true), {
  initialize: function(){
    this.buffer = [];
  },
...
PhoneScheme.Port.StringInput = PhoneScheme.Class.extend(new PhoneScheme.Port(true, false), {
  initialize: function(str){
    this.str = str;
  },
...
PhoneScheme.Port.NullInput = PhoneScheme.Class.extend(new PhoneScheme.Port(true, false), {
  initialize: function(){
  },
  get_string: function(after){
...
PhoneScheme.Port.NullOutput = PhoneScheme.Class.extend(new PhoneScheme.Port(false, true), {
...
PhoneScheme.Port.CustomOutput = PhoneScheme.Class.extend(new PhoneScheme.Port(false, true), {
  initialize: function(output_function){
    this.output_function = output_function;
  },
  put_string: function(str){
...
PhoneScheme.Port.CustomInput = PhoneScheme.Class.extend(new PhoneScheme.Port(true, false), {
  initialize: function(input_function){
    this.input_function = input_function;
  },
  get_string: function(after){
    var input_function = this.input_function;
    return new PhoneScheme.Pause(function(pause) {
...
PhoneScheme.Port.current_input  = new PhoneScheme.Port.NullInput();
PhoneScheme.Port.current_output = new PhoneScheme.Port.NullOutput();
PhoneScheme.Port.current_error  = new PhoneScheme.Port.NullOutput();
//
// R6RS Records
...
//
PhoneScheme.Record = PhoneScheme.Class.create({
  initialize: function(rtd, values){
    PhoneScheme.assert_record_td(rtd, "new Record");
  ...
  toString: function(){
    var contents = PhoneScheme.to_write(this.fields);
    return "#<Record "+this.rtd.name+" "+contents+">";
  }
});

PhoneScheme.isRecord = function(o){
  return (o instanceof PhoneScheme.Record);
};

// Defined record types
PhoneScheme.Record._DefinedTypes = {};

PhoneScheme.Record.define_type = function(name_str, rtd, cd){
  return PhoneScheme.Record._DefinedTypes[name_str] = {rtd: rtd, cd: cd};
};
PhoneScheme.Record.get_type = function(name_str){
  return PhoneScheme.Record._DefinedTypes[name_str];
};

//
// RTD (Record type descriptor)
//
PhoneScheme.Record.RTD = PhoneScheme.Class.create({
  ...
  _generate_new_uid: function(){
    return PhoneScheme.Sym(_.uniqueId("__record_td_uid"));
  },
...
PhoneScheme.Record.RTD.NongenerativeRecords = {};
PhoneScheme.isRecordTD = function(o){
  return (o instanceof PhoneScheme.Record.RTD);
};

//
// CD (Record constructor descriptor)
//
PhoneScheme.Record.CD = PhoneScheme.Class.create({
  ...
  _default_protocol_for_base_types: function(){
    ...
    return function(ar){
      var p = ar[0];
      PhoneScheme.assert_procedure(p, "_default_protocol/base");
      return p;
    };
  },
  ...
  _default_protocol_for_derived_types: function(){
    ...
    return function(ar){
      var n = ar[0];
      PhoneScheme.assert_procedure(n, "_default_protocol/n");
      ...
      var ctor = function(args){
        ...
        // (n a b x y) => p
        return new PhoneScheme.Call(n, ancestor_values, function(ar){
          var p = ar[0];
          PhoneScheme.assert_procedure(p, "_default_protocol/p");

          // (p s t) => record
          return new PhoneScheme.Call(p, my_values, function(ar){
            var record = ar[0];
            PhoneScheme.assert_record(record, "_default_protocol/result");
  ...
  record_constructor: function(){
    ...
    return new PhoneScheme.Call(this.protocol, [arg_for_protocol], function(ar){
      var ctor = ar[0];
      PhoneScheme.assert_procedure(ctor, "record_constructor");
      return ctor;
    });
  },
  ...
  _make_p: function(){
    return function(values){
      return new PhoneScheme.Record(this.rtd, values);
      // TODO: check argc 
    };
  },
    ...
    if(parent_cd){
      ...
      var n = function(args_for_n){
        ...
        var p = function(args_for_p){
          ...
          return new PhoneScheme.Call(parent_cd.protocol, [parent_n], function(ar){
            var ctor = ar[0];
            PhoneScheme.assert_procedure(ctor, "_make_n");

            return new PhoneScheme.Call(ctor, args_for_n, function(ar){
              var record = ar[0];
              PhoneScheme.assert_record(record);
              return record;
            });
          });
        };
      ...
      var n = function(my_values){
        ...
        return new PhoneScheme.Record(rtd, values);
        // TODO: check argc 
      };
      return n;
    }
...
PhoneScheme.isRecordCD = function(o){
  return (o instanceof PhoneScheme.Record.CD);
};
// 
// R6RS Enumerations
...
//
PhoneScheme.Enumeration.EnumType = PhoneScheme.Class.create({
  ...
  // Returns an EnumSet.
  universe: function(){
    return new PhoneScheme.Enumeration.EnumSet(this, this.members);
  }, 
  ...
  indexer: function(){
    ...
    return _.bind(function(ar){
      PhoneScheme.assert_symbol(ar[0], "(enum-set indexer)");
  ...
  constructor: function(){
    ...
    // Returns a enum_set.
    return _.bind(function(ar){
      PhoneScheme.assert_list(ar[0], "(enum-set constructor)");
      var symbols = ar[0].to_array();
      _.each(symbols, function(arg){
        PhoneScheme.assert_symbol(arg, "(enum-set constructor)");
      });

      return new PhoneScheme.Enumeration.EnumSet(this, symbols);
    }, this);
  }
});
PhoneScheme.Class.memoize(PhoneScheme.Enumeration.EnumType,
...
//
PhoneScheme.Enumeration.EnumSet = PhoneScheme.Class.create({
  ...
  symbol_list: function(){
    return PhoneScheme.array_to_list(this.symbols); 
  },
  ...
  union: function(other){
    ...
    return new PhoneScheme.Enumeration.EnumSet(this.enum_type, syms);
  },
  ...
  projection: function(other){
    ...
    return new PhoneScheme.Enumeration.EnumSet(other.enum_type, syms);
  },

  // Returns a string which represents the enum_set.
  toString: function(){
    return "#<EnumSet "+PhoneScheme.inspect(this.symbols)+">";
  }
});
PhoneScheme.Class.memoize(PhoneScheme.Enumeration.EnumSet, "symbol_list");

PhoneScheme.isEnumSet = function(obj){
  return (obj instanceof PhoneScheme.Enumeration.EnumSet);
};
//
// Hashtable
//
...
PhoneScheme.Hashtable = PhoneScheme.Class.create({
  ...
  remove_pair: function(hashed, pair){
    ...
    if (i == -1){
      throw new PhoneScheme.Bug("Hashtable#remove_pair: pair not found!");
    }
    else {
  ...
  create_copy: function(mutable){
    var copy = new PhoneScheme.Hashtable(this.hash_proc, this.equiv_proc,
                                        mutable);
...
PhoneScheme.Hashtable.equal_hash = function(ar){
  return PhoneScheme.to_write(ar[0]);
};
PhoneScheme.Hashtable.eq_hash = PhoneScheme.Hashtable.equal_hash;
PhoneScheme.Hashtable.eqv_hash = PhoneScheme.Hashtable.equal_hash;

PhoneScheme.Hashtable.string_hash = function(ar){
  return ar[0];
};

PhoneScheme.Hashtable.string_ci_hash = function(ar){
  return _.isString(ar[0]) ? ar[0].toLowerCase() : ar[0];
};

PhoneScheme.Hashtable.symbol_hash = function(ar){
  return (ar[0] instanceof PhoneScheme.Symbol) ? ar[0].name : ar[0];
};
...
PhoneScheme.Hashtable.eq_equiv = function(ar){
  return PhoneScheme.eq(ar[0], ar[1]);
};

PhoneScheme.Hashtable.eqv_equiv = function(ar){
  return PhoneScheme.eqv(ar[0], ar[1]);
};
//
// Syntax
//
PhoneScheme.Syntax = PhoneScheme.Class.create({
  ...
  transform: function(x){
    if (!this.func){
      throw new PhoneScheme.Bug("sorry, syntax "+this.sname+
...
PhoneScheme.CoreEnv["define"] = new PhoneScheme.Syntax("define");
PhoneScheme.CoreEnv["begin"]  = new PhoneScheme.Syntax("begin");
PhoneScheme.CoreEnv["quote"]  = new PhoneScheme.Syntax("quote");
PhoneScheme.CoreEnv["lambda"] = new PhoneScheme.Syntax("lambda");
PhoneScheme.CoreEnv["if"]     = new PhoneScheme.Syntax("if");
PhoneScheme.CoreEnv["set!"]   = new PhoneScheme.Syntax("set!");
  //
  // Parser 
  // copied from jsScheme - should be rewrriten (support #0=, etc)
  //
  PhoneScheme.Parser = PhoneScheme.Class.create({
    ...
    inspect: function(){
      return [
        ...
        PhoneScheme.inspect(this.tokens),
        ">"
      ].join("");
    },
          ...
          if (t == "#|") {
            ...
            if ( /(.*\|#)/.test(t) ) {
              ...
              if (in_srfi_30_comment < 0) {
                throw new PhoneScheme.Error("Found an extra comment terminator: `|#'")
              }
    ...
    getObject: function() {
      ...
      r = this.getObject();
      if (r == PhoneScheme.Parser.EOS)
        throw new PhoneScheme.Error("Readable object not found after S exression comment");
    ...
    getList: function( close ) {
      var list = PhoneScheme.nil, prev = list;
      while( this.i < this.tokens.length ) {
        ...
        if( this.tokens[ this.i ] == '.' ) {
          ...
          if( o != PhoneScheme.Parser.EOS && list != PhoneScheme.nil ) {
            prev.cdr = o;
          }
        } else {
            var cur = new PhoneScheme.Pair( this.getObject(), PhoneScheme.nil);
            if( list == PhoneScheme.nil ) list = cur;
            else prev.cdr = cur;
            prev = cur;
        }
      }
    ...
    eatObjectsInSexpComment: function(err_msg) {
      while( this.tokens[ this.i ] == '#;' ) {
        this.i++;
        if ((this.getObject() == PhoneScheme.Parser.EOS) || (this.i >= this.tokens.length))
          throw new PhoneScheme.Error(err_msg);  
      }
    }, 

    getObject0: function() {
      if( this.i >= this.tokens.length )
        return PhoneScheme.Parser.EOS;
      ...
      if( s || t == '(' || t == '#(' || t == '[' || t == '#[' || t == '{' || t == '#{' ) {
        return s ? new PhoneScheme.Pair( PhoneScheme.Sym(s), new PhoneScheme.Pair( this.getObject(), PhoneScheme.nil ))
        ...
        if( ! isNaN(n) ) {
          ...
          // NOTE: This violates R6RS which requires the full Unicode range!
            ...
            throw new PhoneScheme.Error("Character literal out of range.");
          } else {
            return PhoneScheme.Char.get(String.fromCharCode(scalar));
          }
        } else if( /^\"(\\(.|$)|[^\"\\])*\"?$/.test(t) ) {
          return t.replace(/(\r?\n|\\n)/g, "\n").replace( /^\"|\\(.|$)|\"$/g, function($0,$1) {
            return $1 ? $1 : '';
          } );
        } else return PhoneScheme.Sym(t);  // 2Do: validate !!
      }
    }
  });
  // indicates end of source file
  PhoneScheme.Parser.EOS = new Object();
...
PhoneScheme.Compiler = PhoneScheme.Class.create({
  initialize: function(){
  },
  ...
  compile_lookup: function(x, e, return_local, return_free, return_global){
    ...
    //throw new PhoneScheme.Error("undefined symbol `" + sym + "'");
  },
  ...
  make_boxes: function(sets, vars, next){
    ...
    var n = 0;
    var a = [];
    while(vars instanceof PhoneScheme.Pair){
  ...
  find_sets: function(x, v){
    ...
    var ret=null;
    if(x instanceof PhoneScheme.Symbol){
      ret = new PhoneScheme.Set();
    }
    else if(x instanceof PhoneScheme.Pair){
      switch(x.first()){
      case PhoneScheme.Sym("define"):
        var exp=x.third();
        ret = this.find_sets(exp, v);
      case PhoneScheme.Sym("begin"):
        ret = this.find_sets(x.cdr, v); //(ignores improper list)
        break;
      case PhoneScheme.Sym("quote"):
        ret = new PhoneScheme.Set();
        break;
      case PhoneScheme.Sym("lambda"):
        var vars=x.second(), body=x.cdr.cdr;
        if (vars instanceof PhoneScheme.Pair){ // (lambda (...) ...)
          ret = this.find_sets(body, v.set_minus(vars.to_set()));
        }
        else { // (lambda args ...)
          ret = this.find_sets(body, v.set_minus(new PhoneScheme.Set(vars)));
        }
        break;
      case PhoneScheme.Sym("if"):
      ...
      case PhoneScheme.Sym("set!"):
        ...
        break;
      case PhoneScheme.Sym("call/cc"):
        var exp=x.second();
        ret = this.find_sets(exp, v);
        break;
      default:
        var set = new PhoneScheme.Set();
        for(var p=x; p instanceof PhoneScheme.Pair; p=p.cdr){
    ...
    if(ret == null)
      throw new PhoneScheme.Bug("find_sets() exited in unusual way");
    else
      return ret;
  },
  ...
  find_free: function(x, b, f){
    ...
    if(ret == null)
      throw new PhoneScheme.Bug("find_free() exited in unusual way");
    else
      return ret;
  },
  ...
  find_dot_pos: function(x){
    var idx = 0;
    for (; x instanceof PhoneScheme.Pair; x = x.cdr, ++idx)
      ;
    if (x != PhoneScheme.nil) {
      return idx;
    } else {
      return -1;
    }
  ...
  last_pair: function(x){
    if (x instanceof PhoneScheme.Pair){
      for (; x.cdr instanceof PhoneScheme.Pair; x = x.cdr)
        ;
    }
    return x;
  },
  ...
  dotted2proper: function(ls){
    if (ls === PhoneScheme.nil) return PhoneScheme.nil;

    var nreverse = function(ls){
      var res = PhoneScheme.nil;
      for (; ls instanceof PhoneScheme.Pair; ){
        var d = ls.cdr;
        ls.cdr = res;
        res = ls;
        ls = d;
    ...
    var copy_list = function(ls){
      var res = PhoneScheme.nil;
      for (; ls instanceof PhoneScheme.Pair; ls = ls.cdr){
        res = new PhoneScheme.Pair(ls.car, res);
      }
      return nreverse(res);
    }

    if (ls instanceof PhoneScheme.Pair) {
      var last = this.last_pair(ls);
      if (last instanceof PhoneScheme.Pair && last.cdr === PhoneScheme.nil){
        return ls;
      } else {
        var copied = copy_list(ls);
        this.last_pair(copied).cdr = new PhoneScheme.Pair(last.cdr, PhoneScheme.nil);
        return copied;
      }
    } else {
      return new PhoneScheme.Pair(ls, PhoneScheme.nil);
    }
  },
  ...
  compile: function(x, e, s, f, next){
    ...
    while(1){
      if(x instanceof PhoneScheme.Symbol){
        ...
        case PhoneScheme.Sym("begin"):
          var a = [];
          for(var p=x.cdr; p instanceof PhoneScheme.Pair; p=p.cdr)
            a.push(p.car);
        ...
        case PhoneScheme.Sym("quote"):
          if(x.length() < 2)
              throw new PhoneScheme.Error("Invalid quote: "+x.to_write());
        ...
        case PhoneScheme.Sym("if"):
          if(x.length() < 3 || x.length() > 4)
              throw new PhoneScheme.Error("Invalid if: "+x.to_write());
        ...
        case PhoneScheme.Sym("set!"):
          ...
          if(x.length() != 3)
              throw new PhoneScheme.Error("Invalid set!: "+x.to_write());
        ...
        default: 
          ...
          for(var p=args; p instanceof PhoneScheme.Pair; p=p.cdr){
...
//      if(ret == null)
//        throw new PhoneScheme.Bug("compile() exited in unusual way");
//      else
//        return ret;
  },
  ...
  _compile_define: function(x, next){
    if(x.length() == 1) { // 0. (define)
      throw new PhoneScheme.Error("Invalid `define': "+x.to_write());
    }

    var first = x.cdr.car;
    var rest = x.cdr.cdr;
    
    if(first instanceof PhoneScheme.Symbol){    
      if (rest === PhoneScheme.nil) { // 1. (define a)
        x = PhoneScheme.undef;
      }
      else {
        if (rest.cdr === PhoneScheme.nil) // 2. (define a 1)
          x = rest.car;
        else // 3. (define a 1 2)
          throw new PhoneScheme.Error("Invalid `define': "+x.to_write());
      }

      if (!PhoneScheme.TopEnv.hasOwnProperty(first.name)) {
        PhoneScheme.TopEnv[first.name] = PhoneScheme.undef;
      }
      next = ["assign-global", first.name, next];
    }
    else if(first instanceof PhoneScheme.Pair){ // 4. (define (f x) ...)
      ...
      var lambda = new PhoneScheme.Pair(PhoneScheme.Sym("lambda"), new PhoneScheme.Pair(args, rest));
      x = lambda;
      if (!PhoneScheme.TopEnv.hasOwnProperty(first.name)) {
        PhoneScheme.TopEnv[fname.name] = PhoneScheme.undef;
      }
      next = ["assign-global", fname.name, next];
    }
    else{ // 5. (define 1 2)
      throw new PhoneScheme.Error("define: symbol or pair expected but got "+first);
    }
  ...
  _compile_lambda: function(x, e, s, f, next){
    if(x.length() < 3)
      throw new PhoneScheme.Error("Invalid lambda: "+x.to_write());

    var vars = x.cdr.car;
    var body = x.cdr.cdr;

    // Handle internal defines
    var tbody = PhoneScheme.Compiler.transform_internal_define(body);
    if(PhoneScheme.isPair(tbody) &&
       PhoneScheme.isSymbol(tbody.car) &&
       tbody.car.name == "letrec*"){
      // The body has internal defines.
      // Expand letrec* macro
      var cbody = PhoneScheme.Interpreter.expand(tbody);
    }
    else{
      // The body has no internal defines.
      // Just wrap the list with begin 
      var cbody = new PhoneScheme.Pair(PhoneScheme.Sym("begin"), x.cdr.cdr);
    }
    ...
    var do_body = this.compile(cbody,
                    ...
                    ["return"]);
    var do_close = ["close",
                     vars instanceof PhoneScheme.Pair ? vars.length() : 0,
                     free.size(),
  ...
  run: function(expr){
    return this.compile(expr, [new PhoneScheme.Set(), new PhoneScheme.Set()], new PhoneScheme.Set(), new PhoneScheme.Set(), ["halt"]);
  }
});

// Compile an expression with new compiler
PhoneScheme.Compiler.compile = function(expr, next){
  expr = PhoneScheme.Interpreter.expand(expr);
  return (new PhoneScheme.Compiler).run(expr, next);
};
...
var is_definition = function(x){
  return PhoneScheme.isPair(x) &&
         PhoneScheme.Sym("define") === x.car;
...
var define_to_lambda_bind = function(def){
  ...
  if (PhoneScheme.isSymbol(sig)) {
    var variable = sig;

    return new PhoneScheme.Pair(variable, body);
  }
  else {
    var variable = sig.car;
    var value = new PhoneScheme.Pair(PhoneScheme.Sym("lambda"),
                  new PhoneScheme.Pair(sig.cdr, body));

    return PhoneScheme.List(variable, value);
  }
};

PhoneScheme.Compiler.transform_internal_define = function(x){
  ...
  var bindings = PhoneScheme.List.apply(null, _.map(defs, define_to_lambda_bind));
  return new PhoneScheme.Pair(PhoneScheme.Sym("letrec*"),
           new PhoneScheme.Pair(bindings, exprs));
};
})();
//
// pause object (facility to stop/resume interpreting)
//
PhoneScheme.Pause = PhoneScheme.Class.create({
...
//
PhoneScheme.Call = PhoneScheme.Class.create({
...
//
PhoneScheme.Iterator = {
  ForArray: PhoneScheme.Class.create({
  ...
  ForString: PhoneScheme.Class.create({
    initialize: function(str){
      this.str = str;
      this.i = 0;
    },
    ...
    next: function(){
      return PhoneScheme.Char.get(this.str.charAt(this.i++));
    }
  }),
  ForList: PhoneScheme.Class.create({
    initialize: function(ls){
      this.ls = ls;
    },
    has_next: function(){
      return (this.ls instanceof PhoneScheme.Pair) &&
             this.ls != PhoneScheme.nil;
    },
    next: function(){
      var pair = this.ls;
      this.ls = this.ls.cdr;
  ...
  ForMulti: PhoneScheme.Class.create({
    initialize: function(objs){
      this.objs = objs;
      this.size = objs.length;
      this.iterators = _.map(objs, function(x){
        return PhoneScheme.Iterator.of(x);
      })
    },
    has_next: function(){
  ...
  of: function(obj){
    switch(true){
      ...
      case (obj instanceof PhoneScheme.Pair):
      case (obj === PhoneScheme.nil):
        return new this.ForList(obj);
      default:
        throw new PhoneScheme.Bug("Iterator.of: unknown class: "+PhoneScheme.inspect(obj));
    }
  }
}
...
PhoneScheme.Call.default_callbacks = {
  call: function(x){ return new PhoneScheme.Call(this.proc, [x]) },
  result: function(){},
  finish: function(){}
}
PhoneScheme.Call.foreach = function(obj, callbacks, is_multi){
  is_multi || (is_multi = false);
  _.each(["call", "result", "finish"], function(key){
    if(!callbacks[key])
      callbacks[key] = PhoneScheme.Call.default_callbacks[key];
  })
  ...
  var loop = function(ar){
    if(iterator){
      ...
      if(is_multi)
        iterator = new PhoneScheme.Iterator.ForMulti(obj);
      else
        iterator = PhoneScheme.Iterator.of(obj);
    }
...
PhoneScheme.Call.multi_foreach = function(obj, callbacks){
  return PhoneScheme.Call.foreach(obj, callbacks, true);
}
...
PhoneScheme.Interpreter = PhoneScheme.Class.create({
  // new PhoneScheme.Interpreter()
  // new PhoneScheme.Interpreter(lastInterpreter)
  // new PhoneScheme.Interpreter(errorHandler)
  // new PhoneScheme.Interpreter(lastInterpreter, errorHandler)
  initialize: function(){
    ...
    } else if (arguments.length == 1 && arguments[0] instanceof PhoneScheme.Interpreter) {
      last_interpreter = arguments[0];
    ...
    this.max_trace_size = last_interpreter ? last_interpreter.max_trace_size : PhoneScheme.max_trace_size;

    // dynamic-wind
    this.current_dynamic_winder = PhoneScheme.Interpreter.DynamicWind.ROOT;
  },
  ...
  inspect: function(){
    return [
      ...
      this.stack.length, " ",
      "after_evaluate=",
      PhoneScheme.inspect(this.after_evaluate),
      ">"
    ].join("");
  },
  ...
  run_dump_hook: function(a, x, f, c, s) {
    ...
    else if (PhoneScheme.Interpreter.dumper) {
      dumper = PhoneScheme.Interpreter.dumper;
    }
    else
      return;
  ...
  _execute: function(a, x, f, c, s){
    ...
    while(true){ //x[0] != "halt"){
      ...
      case "refer-global":
        var sym=x[1], x=x[2];
        if(PhoneScheme.TopEnv.hasOwnProperty(sym))
          var val = PhoneScheme.TopEnv[sym];
        else if(PhoneScheme.CoreEnv.hasOwnProperty(sym))
          var val = PhoneScheme.CoreEnv[sym];
        else
          throw new PhoneScheme.Error("execute: unbound symbol: "+PhoneScheme.inspect(sym));
      ...
      case "assign-global":
        var name=x[1], x=x[2];
        if(!PhoneScheme.TopEnv.hasOwnProperty(name) &&
           !PhoneScheme.CoreEnv.hasOwnProperty(name))
          throw new PhoneScheme.Error("global variable '"+name+"' is not defined");
        
        PhoneScheme.TopEnv[name] = a;
        a = PhoneScheme.undef;
        break;
      case "assign-local":
        var n=x[1], x=x[2];
      ...
      case "nuate1":
        ...
        var winders = PhoneScheme.Interpreter.DynamicWind.listWinders(from, to);
        x = PhoneScheme.Interpreter.DynamicWind.joinWinders(winders,
          ["nuate2", stack]
        )
        break;
      case "nuate2":
      ...
      case "apply": //extended: n_args as second argument
        ...
        if(PhoneScheme.isClosure(func)){
          ...
          if (dotpos >= 0) {
            ...
            var ls = PhoneScheme.nil;
            for (var i=n_args; --i>=dotpos; ) {
              ls = new PhoneScheme.Pair(this.index(s, i+1), ls);
            }
            if (dotpos >= n_args) {
            ...
            if(func.expected_args && n_args != func.expected_args) {
              ...
              throw new PhoneScheme.Error(errMsg);
            }
          }
          f = s;
          c = a;
          ...
          if(result instanceof PhoneScheme.Pause){
          ...
          else if(result instanceof PhoneScheme.Call){
          ...
          throw new PhoneScheme.Error(PhoneScheme.inspect(func) + " is not a function");
        }
        break;
      case "return":
        // Pop stack frame
        ...
        break;
      default:
        throw new PhoneScheme.Bug("unknown opecode type: "+x[0]);
      }
    }

//      if(ret === null)
//        throw new PhoneScheme.Bug("interpreter exited in unusual way");
//      else
//        return ret;
    return a
  },
  ...
  evaluate: function(str, after_evaluate){
    this.call_stack = [];
    this.parser = new PhoneScheme.Parser(str);
    this.compiler = new PhoneScheme.Compiler();
    if(after_evaluate) 
      this.after_evaluate = after_evaluate;

    if(PhoneScheme.Debug) Console.puts("executing: " + str);
  ...
  resume: function(is_resume, a, x, f, c, s){
    ...
    for(;;){
      if(is_resume){
        ...
        if(expr === PhoneScheme.Parser.EOS) break;
        ...
        //if(PhoneScheme.Debug) Console.p(opc);

        // execute
        ret = this._execute(expr, opc, 0, [], 0);
      }

      if(ret instanceof PhoneScheme.Pause){ //suspend evaluation
        return ret;
      }
    }
  ...
  compile: function(str){
    var obj = PhoneScheme.Interpreter.read(str);
    var opc = PhoneScheme.Compiler.compile(obj);
    return opc;
  },
  ...
  push_dynamic_winder: function(before, after) {
    this.current_dynamic_winder =
      new PhoneScheme.Interpreter.DynamicWind(this.current_dynamic_winder, before, after);
  },
...
PhoneScheme.Interpreter.read = function(str){
  var parser = new PhoneScheme.Parser(str);
  var r      = parser.getObject();
  return (r == PhoneScheme.Parser.EOS)? PhoneScheme.eof: r;
};
...
//
// @throws {PhoneScheme.Error} when x has syntax error
PhoneScheme.Interpreter.expand = function(x, flag/*optional*/){
  var expand = PhoneScheme.Interpreter.expand;
  flag || (flag = {})
  var ret = null;
  ...
  if(x instanceof PhoneScheme.Pair){
    ...
    case PhoneScheme.Sym("define"):
      ...
      ret = new PhoneScheme.Pair(PhoneScheme.Sym("define"),
              new PhoneScheme.Pair(left, expand(exp, flag)));
      break;
    case PhoneScheme.Sym("begin"):
      ret = new PhoneScheme.Pair(PhoneScheme.Sym("begin"), expand(x.cdr, flag));
      break;
    case PhoneScheme.Sym("quote"):
      ret = x;
      break;
    case PhoneScheme.Sym("lambda"):
      ...
      ret = new PhoneScheme.Pair(PhoneScheme.Sym("lambda"),
              new PhoneScheme.Pair(vars, expand(body, flag)));
      break;
    case PhoneScheme.Sym("if"):
      var testc=x.second(), thenc=x.third(), elsec=x.fourth();
      ret = PhoneScheme.List(PhoneScheme.Sym("if"),
                            expand(testc, flag),
                            expand(thenc, flag),
                            expand(elsec, flag));
      break;
    case PhoneScheme.Sym("set!"):
      var v=x.second(), x=x.third();
      ret = PhoneScheme.List(PhoneScheme.Sym("set!"), v, expand(x, flag));
      break;
    case PhoneScheme.Sym("call-with-current-continuation"): 
    case PhoneScheme.Sym("call/cc"): 
      var x=x.second();
      ret = PhoneScheme.List(PhoneScheme.Sym("call/cc"), expand(x, flag));
      break;
    default: //apply
      var transformer = null;
      if(PhoneScheme.isSymbol(x.car)){
        if(PhoneScheme.TopEnv[x.car.name] instanceof PhoneScheme.Syntax)
          transformer = PhoneScheme.TopEnv[x.car.name];
        else if(PhoneScheme.CoreEnv[x.car.name] instanceof PhoneScheme.Syntax)
          transformer = PhoneScheme.CoreEnv[x.car.name];
      }
...
//            // Debug
//            var before = PhoneScheme.to_write(x);
//            var after = PhoneScheme.to_write(ret);
//            if(before != after){
...
//            }
        ...
        var expanded_cdr;
        if(!(x.cdr instanceof PhoneScheme.Pair) && (x.cdr !== PhoneScheme.nil)){
          throw new Error("proper list required for function application "+
                          "or macro use: "+PhoneScheme.to_write(x));
        }
        expanded_cdr = PhoneScheme.array_to_list(
                         _.map(x.cdr.to_array(),
                               function(item){ return expand(item, flag); }));
        ret = new PhoneScheme.Pair(expanded_car, expanded_cdr);
      }
    }
  }
  else{
...
PhoneScheme.Interpreter.DynamicWind = PhoneScheme.Class.create({
...
PhoneScheme.Interpreter.DynamicWind.ROOT = {_: "this is ROOT."};

// Return the list of winders to call
PhoneScheme.Interpreter.DynamicWind.listWinders = function(from, to) {
  // List winders from `from` to `ROOT`
  var fromStack = [from];
  while (from !== PhoneScheme.Interpreter.DynamicWind.ROOT) {
    from = from.parent;
    fromStack.push(from);
  }
...
PhoneScheme.Interpreter.DynamicWind.joinWinders = function(winders, x) {
...
//
PhoneScheme.Promise = PhoneScheme.Class.create({
  ...
  value: function() {
    if (!this.is_done()) {
      throw new PhoneScheme.Bug("this promise is not calculated yet");
    }
    return this.box[1];
  },
  ...
  thunk: function() {
    if (this.is_done()) {
      throw new PhoneScheme.Bug("this promise does not know the thunk");
    }
    return this.box[1];
  },
...
PhoneScheme.isPromise = function(obj) {
  return (obj instanceof PhoneScheme.Promise);
};

// Create fresh promise
PhoneScheme.Promise.fresh = function(thunk) {
  return new PhoneScheme.Promise(false, thunk);
};
// Create calculated promise
PhoneScheme.Promise.done = function(value) {
  return new PhoneScheme.Promise(true, value);
};
///
...
//
PhoneScheme.check_arity = function(len, min, max){
  var fname = arguments.callee.caller
                ? arguments.callee.caller.fname
                : "(?)";
  if(len < min){
    if(max && max == min)
      throw new PhoneScheme.Error(fname+": wrong number of arguments (expected: "+min+" got: "+len+")");
    else
      throw new PhoneScheme.Error(fname+": too few arguments (at least: "+min+" got: "+len+")");
  }
  else if(max && max < len)
    throw new PhoneScheme.Error(fname+": too many arguments (at most: "+max+" got: "+len+")");
}
PhoneScheme.define_libfunc = function(fname, min, max, func){
  var f = function(ar, intp){
    PhoneScheme.check_arity(ar.length, min, max);
    return func(ar, intp);
  };
  ...
  PhoneScheme.CoreEnv[fname] = f;
}
PhoneScheme.alias_libfunc = function(fname, aliases) {
  if (PhoneScheme.CoreEnv[fname]) {
    if (_.isArray(aliases)) {
      _.map(aliases, function(a) { PhoneScheme.alias_libfunc(fname, a); });
    } else if (_.isString(aliases)) {
      PhoneScheme.CoreEnv[aliases] = PhoneScheme.CoreEnv[fname];
    } else {
...
PhoneScheme.define_syntax = function(sname, func) {
  var s = new PhoneScheme.Syntax(sname, func);
  PhoneScheme.CoreEnv[sname] = s;
}
PhoneScheme.define_scmfunc = function(fname, min, max, str){
  (new PhoneScheme.Interpreter).evaluate("(define "+fname+" "+str+"\n)");
}
...
//
PhoneScheme.make_assert = function(check){
...
PhoneScheme.make_simple_assert = function(type, test, _fname){
  return PhoneScheme.make_assert(function(fname, obj, opt){
    if(_fname) fname = _fname;
    option = opt ? ("("+opt+")") : ""
    if(!test(obj)){
      throw new PhoneScheme.Error(fname + option + ": " +
                                 type + " required, but got " +
                                 PhoneScheme.to_write(obj));
    }
  })
}

PhoneScheme.assert_number = PhoneScheme.make_simple_assert("number", function(obj){
  return typeof(obj) == 'number' || (obj instanceof PhoneScheme.Complex);
});

PhoneScheme.assert_integer = PhoneScheme.make_simple_assert("integer", function(obj){
  return typeof(obj) == 'number' && (obj % 1 == 0)
});

PhoneScheme.assert_real = PhoneScheme.make_simple_assert("real number", function(obj){
  return typeof(obj) == 'number';
});

PhoneScheme.assert_between = PhoneScheme.make_assert(function(fname, obj, from, to){
  if( typeof(obj) != 'number' || obj != Math.round(obj) ){
    throw new PhoneScheme.Error(fname + ": " +
...
PhoneScheme.assert_string = PhoneScheme.make_simple_assert("string", _.isString);

PhoneScheme.assert_char = PhoneScheme.make_simple_assert("character", PhoneScheme.isChar);
PhoneScheme.assert_symbol = PhoneScheme.make_simple_assert("symbol", PhoneScheme.isSymbol);
PhoneScheme.assert_port = PhoneScheme.make_simple_assert("port", PhoneScheme.isPort);
PhoneScheme.assert_pair = PhoneScheme.make_simple_assert("pair", PhoneScheme.isPair);
PhoneScheme.assert_list = PhoneScheme.make_simple_assert("list", PhoneScheme.isList);
PhoneScheme.assert_vector = PhoneScheme.make_simple_assert("vector", PhoneScheme.isVector);

PhoneScheme.assert_hashtable = PhoneScheme.make_simple_assert("hashtable",
                                          PhoneScheme.isHashtable);
PhoneScheme.assert_mutable_hashtable = PhoneScheme.make_simple_assert("mutable hashtable",
                                            PhoneScheme.isMutableHashtable);

PhoneScheme.assert_record = PhoneScheme.make_simple_assert("record",
                                          PhoneScheme.isRecord);
PhoneScheme.assert_record_td = PhoneScheme.make_simple_assert("record type descriptor",
                                          PhoneScheme.isRecordTD);
PhoneScheme.assert_record_cd = PhoneScheme.make_simple_assert("record constructor descriptor",
                                          PhoneScheme.isRecordCD);
PhoneScheme.assert_enum_set = PhoneScheme.make_simple_assert("enum_set",
                                          PhoneScheme.isEnumSet);
PhoneScheme.assert_promise = PhoneScheme.make_simple_assert("promise",
                                          PhoneScheme.isPromise);

PhoneScheme.assert_function = PhoneScheme.make_simple_assert("JavaScript function",
                                         _.isFunction);
PhoneScheme.assert_closure = PhoneScheme.make_simple_assert("scheme function",
                                        PhoneScheme.isClosure);
PhoneScheme.assert_procedure = PhoneScheme.make_simple_assert("scheme/js function", function(obj){
  return PhoneScheme.isClosure(obj) || _.isFunction(obj);
});

PhoneScheme.assert_date = PhoneScheme.make_simple_assert("date", function(obj){
  // FIXME: this is not accurate (about cross-frame issue)
  // https://prototype.lighthouseapp.com/projects/8886/tickets/443
  return obj instanceof Date;
});

//var assert_instance_of = PhoneScheme.make_assert(function(fname, type, obj, klass){
...
PhoneScheme.assert = PhoneScheme.make_assert(function(fname, success, message, _fname){
  if(!success){
    throw new PhoneScheme.Error((_fname || fname)+": "+message);
  }
});
...
PhoneScheme.deprecate = function(title, ver, alt){
  if(PhoneScheme.suppress_deprecation_warning) return;

  var msg = title+" is deprecated and will be removed in PhoneScheme "+ver+ ". "+
...
// @return {float|false}
PhoneScheme.parse_fraction = function(rep) {
  PhoneScheme.assert_string(rep);
  ...
  var num = PhoneScheme.parse_integer(num_rep, 10);
  var denom = PhoneScheme.parse_integer(denom_rep, 10);
...
// @return {boolean}
PhoneScheme.is_valid_integer_notation = function(rep, rdx) {
  PhoneScheme.assert_string(rep);
  PhoneScheme.assert_integer(rdx);
...
PhoneScheme.parse_integer = function(rep, rdx) {
  PhoneScheme.assert_string(rep);
  PhoneScheme.assert_integer(rdx);
  ...
  if (!PhoneScheme.is_valid_integer_notation(rep, rdx))
    return false;
...
// @return {boolean}
PhoneScheme.is_valid_float_notation = function(rep) {
  PhoneScheme.assert_string(rep);
  ...
  return PhoneScheme.is_valid_integer_notation(rep, 10);
};
...
// @return {float|false}
PhoneScheme.parse_float = function(rep) {
  PhoneScheme.assert_string(rep);

  if (!PhoneScheme.is_valid_float_notation(rep))
    return false;
...
if( typeof(PhoneScheme)!='object' ) PhoneScheme={}; with(PhoneScheme) {
  ///
  /// R6RS Base library
  ///
    ...
    _.each(clauses.to_array().reverse(), function(clause){
      ...
      if(clause.car === Sym("else")){
        ...
        if(clause.cdr === nil){
          ...
          var tmp_sym = PhoneScheme.gensym();
  ...
  define_syntax("let", function(x){
    ...
    if((!(binds instanceof Pair)) && binds != PhoneScheme.nil){
  ...
  var expand_letrec_star = function(x){
    ...
    _.each(binds.to_array().reverse(), function(bind){
      letbody = new Pair(new Pair(bind.car,
                           new Pair(PhoneScheme.undef, nil)),
                  letbody);
    })
  ...
  define_libfunc("equal?", 2, 2, function(ar){
    return PhoneScheme.equal(ar[0], ar[1]);
  })
  ...
  define_libfunc("procedure?", 1, 1, function(ar){
    return PhoneScheme.isProcedure(ar[0]);
  })
  ...
  define_libfunc("number?", 1, 1, function(ar){
    return PhoneScheme.isNumber(ar[0]);
  });
  define_libfunc("complex?", 1, 1, function(ar){
    return PhoneScheme.isComplex(ar[0]);
  });
  define_libfunc("real?", 1, 1, function(ar){
    return PhoneScheme.isReal(ar[0]);
  });
  define_libfunc("rational?", 1, 1, function(ar){
    return PhoneScheme.isRational(ar[0]);
  });
  define_libfunc("integer?", 1, 1, function(ar){
    return PhoneScheme.isInteger(ar[0]);
  });
...
//(rationalize x1 x2)    procedure
  ...
  define_libfunc("string->number", 1, 3, function(ar){
    ...
    var int_res = PhoneScheme.parse_integer(
    ...
    var fp_res = PhoneScheme.parse_float(s);

    if (fp_res !== false)
      return fp_res;

    var frac_res = PhoneScheme.parse_fraction(s);
  ...
  define_libfunc("for-each", 2, null, function(ar){
    ...
    return Call.multi_foreach(lists, {
      ...
      finish: function(){ return PhoneScheme.undef; }
    })
  })
  ...
  define_libfunc("vector?", 1, 1, function(ar){
    return PhoneScheme.isVector(ar[0]);
  })
      ...
      var vectors = vecs.map(function(vec){
        if (vec["splicing"]) {
          ...
          return Cons(Sym("vector"),
                      PhoneScheme.array_to_list(vec));
        }
      });
      if (vectors.length == 1) {
         return Cons(Sym("vector"),
                     PhoneScheme.array_to_list(vecs[0]));
      }
      else {
        return Cons(Sym("vector-append"),
                    PhoneScheme.array_to_list(vectors));
      }
    }
    else
      return f;
...
//(string-foldcase string)    procedure
  ...
  PhoneScheme.make_string_ci_function = function(compare){
    return function(ar){
      assert_string(ar[0]);
...
//       (partition2 proc ls '() '()))");
  ...
  (function(){
    ...
    var compareFn = function(a,b){
      return PhoneScheme.lt(a, b) ? -1 :
             PhoneScheme.lt(b, a) ? 1 : 0;
    };
  ...
  define_syntax("case-lambda", function(x){
    ...
    clauses.reverse().forEach(function(clause) {
      ...
      else if (PhoneScheme.isSymbol(formals)) {
  ...
  define_syntax("define-record-type", function(x){
    ...
    if(PhoneScheme.isSymbol(name_spec)){
      ...
      switch(clause.car){
        ...
        case Sym("fields"):
          fields = _.map(clause.cdr.to_array(), function(field_spec, idx){
            if(PhoneScheme.isSymbol(field_spec)){
              // - <field name>
              ...
              switch(field_spec.car){
                case Sym("immutable"):
                  ...
                  if(PhoneScheme.isNil(field_spec.cdr.cdr))
                ...
                default:
                  throw new Error("define-record-type: field definition "+
                              ...
                              "but got "+PhoneScheme.inspect(field_spec.car));
              }
            }
          });
          break;
        ...
        case Sym("parent-rtd"):
          ...
          break;
        default:
          throw new PhoneScheme.Error("define-record-type: unknown clause `"+
                                     PhoneScheme.to_write(clause.car)+"'");
      }
    });
    ...
    // the hash PhoneScheme.Record._DefinedTypes.
  ...
  define_libfunc("_define-record-type", 3, 3, function(ar){
    ...
    PhoneScheme.Record.define_type(ar[0].name, ar[1], ar[2]);
    return PhoneScheme.undef;
  });
  ...
  define_libfunc("_record-type-descriptor", 1, 1, function(ar){
    assert_symbol(ar[0]);
    var type = PhoneScheme.Record.get_type(ar[0].name);
    if(type)
      return type.rtd;
    else
  ...
  define_libfunc("make-record-type-descriptor", 6, 6, function(ar){
    ...
    if(uid){
      assert_symbol(uid);
      var _rtd = PhoneScheme.Record.RTD.NongenerativeRecords[uid.name];
      if(_rtd){
    ...
    var rtd = new PhoneScheme.Record.RTD(name, parent_rtd, uid,
                                     sealed, opaque, fields);
    if(uid)
      PhoneScheme.Record.RTD.NongenerativeRecords[uid.name] = rtd;
  ...
  define_libfunc("record-type-descriptor?", 1, 1, function(ar){
    return (ar[0] instanceof PhoneScheme.Record.RTD);
  });
  ...
  define_libfunc("make-record-constructor-descriptor", 3, 3, function(ar){
    ...
    return new PhoneScheme.Record.CD(rtd, parent_cd, protocol);
  });
  ...
  define_libfunc("record-predicate", 1, 1, function(ar){
    ...
    return function(args){
      ...
      return (obj instanceof PhoneScheme.Record) &&
             (obj.rtd === rtd);
    };
  });
  ...
  define_libfunc("record-accessor", 2, 2, function(ar){
    ...
    return function(args){
      ...
      var error_msg = rtd.name.name+"-"+rtd.field_name(k)+": "+
                      PhoneScheme.to_write(record)+
                      " is not a "+rtd.name.name;
      assert(PhoneScheme.isRecord(record), error_msg);
  ...
  define_libfunc("record-mutator", 2, 2, function(ar){
    ...
    return function(args){
      ...
      assert(record.rtd === rtd,
            func_name+": "+PhoneScheme.to_write(record)+
            " is not a "+rtd.name.name);
...
//(guard (<variable>    syntax
  ...
  define_libfunc("raise", 1, 1, function(ar){
    throw new PhoneScheme.UserError(PhoneScheme.to_write(ar[0]));
  });
...
//(open-string-output-port)    procedure
  ...
  define_libfunc("call-with-string-output-port", 1, 1, function(ar){
    ...
    var port = new PhoneScheme.Port.StringOutput();
...
//(fixnum->flonum fx)    procedure
  ...
  //               hashed - Object hashed
  //
  // Returns an instance of PhoneScheme.Call.
  PhoneScheme.find_hash_pair = function(hash, key, callbacks){
    // invoke hash proc
  ...
  define_libfunc("hashtable-ref", 3, 3, function(ar){
    ...
    return PhoneScheme.find_hash_pair(hash, key, {
  ...
  define_libfunc("make-enumeration", 1, 1, function(ar){
    ...
    var enum_type = new PhoneScheme.Enumeration.EnumType(members);
  ...
  define_syntax("define-enumeration", function(x){
    ...
    define_syntax(type_name, function(x){
      // (color)
      assert(!PhoneScheme.isNil(x.cdr),
      ...
      assert(_.include(enum_type.members, arg),
        arg.name+" is not included in the universe: "+
          PhoneScheme.to_write(enum_type.members),
        type_name);

      return PhoneScheme.List(Sym("quote"), arg);
    });
    ...
    define_syntax(constructor_name, function(x){
      ...
      // Create an EnumSet
      return new PhoneScheme.Enumeration.EnumSet(enum_type, symbols);
    });
  });
  ...
  define_libfunc("eval", 1, 1, function(ar, intp){
    ...
    return intp2.evaluate(PhoneScheme.to_write(expr));
  });
...
//(scheme-report-environment n)    procedure
  ...
  define_syntax("delay", function(x){
    ...
    return new Pair(Sym(" procedure->promise"),
             new Pair(new Pair(Sym("lambda"),
                        new Pair(PhoneScheme.nil,
                          new Pair(new Pair(Sym("make-promise"),
                                     new Pair(expr, PhoneScheme.nil)),
                            PhoneScheme.nil)))));
  });
  ...
  define_syntax("delay-force", function(x){
    ...
    return new Pair(Sym(" procedure->promise"),
             new Pair(new Pair(Sym("lambda"),
                        new Pair(PhoneScheme.nil,
                          new Pair(expr, PhoneScheme.nil))), PhoneScheme.nil));
  });
  ...
  define_libfunc("promise?", 1, 1, function(ar, intp){
    return (ar[0] instanceof PhoneScheme.Promise);
  });

  // (make-promise obj)
  define_libfunc("make-promise", 1, 1, function(ar, intp){
    var obj = ar[0];
    if (obj instanceof PhoneScheme.Promise) {
      return obj;
    }
    else {
      return PhoneScheme.Promise.done(obj);
    }
  });
  ...
  define_libfunc(" procedure->promise", 1, 1, function(ar, intp){
    assert_procedure(ar[0]);
    return PhoneScheme.Promise.fresh(ar[0]);
  });
}
if( typeof(PhoneScheme)!='object' ) PhoneScheme={}; with(PhoneScheme) {
  ...
  //   if(PhoneScheme.isSymbol(receiver)){
  ...
  //       if(PhoneScheme.isSymbol(callspec)){
  //         // Property access
  //         v = v[callspec.name];
  //       }
  //       else if(PhoneScheme.isList(callspec)){
  //         // Method call
  ...
  //             if(PhoneScheme.isClosure(arg)){
  //               // closure -> JavaScript funciton
  //               return PhoneScheme.js_closure(arg, intp);
  //             }
  //             else if(PhoneScheme.isList(arg)){
  ...
  //           throw new PhoneScheme.Error("js-invocation: the method `"+method+"' not found");
  //         }
  //         v = v[method].apply(v, args);
  //       }
  //       else{
  //         // (wrong argument)
  //         throw new PhoneScheme.Error("js-invocation: expected list or symbol for callspec but got " + PhoneScheme.inspect(callspec));
  //       }
  //     });
  ...
  //       assert_symbol(key);
  //       if(PhoneScheme.isClosure(value))
  //         value = PhoneScheme.js_closure(value, intp);
  ...
  PhoneScheme.js_closure = function(proc, intp){
    var intp2 = new Interpreter(intp);
    return function(/*args*/){
  ...
  //   assert_closure(ar[0]);
  //   return PhoneScheme.js_closure(ar[0], intp);
  // });
  ...
  //   PhoneScheme.deprecate("js-array-to-list", "1.0", "js-array->list");
  //   return PhoneScheme.array_to_list(ar[0]);
  // });
  ...
  //   PhoneScheme.deprecate("list-to-js-array", "1.0", "list->js-array");
  //   return ar[0].to_array();
  // });
  ...
  PhoneScheme.alist_to_js_obj = function(alist) {
    if (alist === nil) {
      return {} ;
    }
    assert_list(alist);
  ...
  //   PhoneScheme.deprecate("alist-to-js-obj", "1.0", "alist->js-obj");
  //   return PhoneScheme.alist_to_js_obj(ar[0]);
  // });
  ...
  PhoneScheme.js_obj_to_alist = function(obj) {
    if (obj === undefined) {
      return PhoneScheme.nil;
    }
    var arr = [];
    _.each(obj, function(val, key) {
      arr.push(new Pair(key, val));
    });
    var alist = PhoneScheme.array_to_list(arr);
    return alist;
  };
  define_libfunc("js-obj-to-alist", 1, 1, function(ar) {
    PhoneScheme.deprecate("js-obj-to-alist", "1.0", "js-obj->alist");
    return PhoneScheme.js_obj_to_alist(ar[0]);
  });
    ...
    define_libfunc("console-"+name, 1, null, function(ar){
      ...
      if(con){
        var vals = _.map(ar, function(item){
          return PhoneScheme.inspect(item, {fallback: item});
        });
...
if( typeof(PhoneScheme)!='object' ) PhoneScheme={}; with(PhoneScheme) {
  ...
  PhoneScheme.inspect_objs = function(objs){
    return _.map(objs, PhoneScheme.inspect).join(", ");
  };
  define_libfunc("inspect", 1, null, function(ar){
    return PhoneScheme.inspect_objs(ar);
  });
  define_libfunc("inspect!", 1, null, function(ar){
    Console.puts(PhoneScheme.inspect_objs(ar));
    return PhoneScheme.undef;
  });
  ...
  //
  PhoneScheme.json2sexp = function(json){
    switch(true){
    ...
    case typeof(json) == "object":
      ...
      return ls;
    default:
      throw new Error("json->sexp: detected invalid value for json: "+PhoneScheme.inspect(json));
    }
  ...
  var sort_with_comp = function(ary, proc, intp){
    return ary.sort(function(a, b){
        var intp2 = new PhoneScheme.Interpreter(intp);
  ...
  define_syntax("define-macro", function(x){
    ...
    var cls = PhoneScheme.makeClosure([opc[3]]);
  ...
  define_syntax("%macroexpand", function(x){
    var expanded = PhoneScheme.Interpreter.expand(x.cdr.car);
  ...
  define_libfunc("macroexpand", 1, 1, function(ar){
    return PhoneScheme.Interpreter.expand(ar[0]);
  });
  ...
  define_libfunc("with-output-to-port", 2, 2, function(ar){
    ...
    var original_port = PhoneScheme.Port.current_output;
    PhoneScheme.Port.current_output = port

    return new Call(proc, [port], function(ar){
      port.close();
      PhoneScheme.Port.current_output = original_port;
...
(function(){
  ...
  var define_node_libfunc = function(/*arguments*/){
    ...
    if(PhoneScheme.on_node){
      PhoneScheme.define_libfunc.apply(null, args);
    }
    else{
      var func_name = args[0];
      var func = function(ar){
        throw new PhoneScheme.Error("the function '"+func_name+"' "+
  ...
  //   PhoneScheme.assert_string(ar[0]);
  ...
  //   return PhoneScheme.List.apply(null, node.process.argv);
  // });
  ...
  //   return PhoneScheme.js_obj_to_alist(node.process.env);
  // });
...
with(PhoneScheme) {
  ...
  var copy_pair = function(pair){
    var car = PhoneScheme.isPair(pair.car) ? copy_pair(pair.car)
                                          : pair.car;
    var cdr = PhoneScheme.isPair(pair.cdr) ? copy_pair(pair.cdr)
                                          : pair.cdr;
    return new Pair(car, cdr);
  };
  // (list-copy list)
  define_libfunc("list-copy", 1, 1, function(ar){
    if(PhoneScheme.isPair(ar[0])){
  ...
  define_syntax("receive", function(x){
    assert(PhoneScheme.isPair(x.cdr),
           "missing formals", "receive");
    var formals = x.cdr.car;
    assert(PhoneScheme.isPair(x.cdr.cdr),
           "missing expression", "receive");
    var expression = x.cdr.cdr.car;
    var body       = x.cdr.cdr.cdr;
    
    return deep_array_to_list([Sym("call-with-values"),
      [Sym("lambda"), PhoneScheme.nil, expression],
      new PhoneScheme.Pair(Sym("lambda"),
        new PhoneScheme.Pair(formals, body))]);
  });
...
//subtract-duration! time1 time-duration -> time
  ...
  PhoneScheme.date2string = function(date, format){
  ...
  define_libfunc("format", 1, null, function(ar){
    if (_.isString(ar[0])) {
      ...
      ar.shift();
      var port = PhoneScheme.Port.current_output,
          format_str = ar.shift();
    }
    else {
    ...
    var str = format_str.replace(/~[as]/g, function(matched){
                 ...
                 if (matched == "~a")
                   return PhoneScheme.to_display(ar.shift());
                 else
                   return PhoneScheme.to_write(ar.shift());
              }).replace(/~%/, "\n")
                .replace(/~~/, "~");
    if (port) {
      port.put_string(str);
...
if( typeof(PhoneScheme)!='object' ) PhoneScheme={}; with(PhoneScheme) {
  ...
  //   PhoneScheme.deprecate("element-write-attribute", "1.0",
  ...
  //   PhoneScheme.deprecate("element-add-class-name", "1.0",
  ...
  //   PhoneScheme.deprecate("element-remove-class-name", "1.0",
  ...
  //   PhoneScheme.deprecate("element-toggle-class-name", "1.0",
  ...
  PhoneScheme.create_elements_by_string = function(spec){
    spec = spec.to_array();
    var name = spec.shift();
  ...
  PhoneScheme.tree_all = function(tree, pred){
    if(tree === nil)
      return true;
    else if(pred(tree.car) === false)
      return false;
    else
      return PhoneScheme.tree_all(tree.cdr, pred); 
  };
  ...
  //   };
  //   if(PhoneScheme.tree_all(ar[0], string_or_symbol)){
  //     return $(create_elements_by_string(ar[0]))[0];
  //   } else {
  //     return nil;
  //   }
  // });
  PhoneScheme.element_content = function(selector) {
  ...
  //   return PhoneScheme.element_content(ar[0]);
  // });
  ...
  //         intp2.evaluate(data,
  //                        function(){
  //                          return pause.resume(PhoneScheme.undef);
  //                        });
  //       },
  //       error: function() {
  ...
  //       pause.resume(PhoneScheme.undef);
  //     });
  //   });
  // });
  ...
  PhoneScheme.getelem = function(ar){
  ...
  // define_libfunc("$",       1, 2, PhoneScheme.getelem);
  // define_libfunc("getelem", 1, 2, PhoneScheme.getelem);
...
//  });
  ...
  PhoneScheme.jsonp_receiver = [];
  // define_libfunc("receive-jsonp", 1, 1, function(ar){
  //   var url = ar[0];
  //   assert_string(url);

  //   var receives = PhoneScheme.jsonp_receiver;
  //   for(var i=0; i<receives.length; i++)
  //     if(receives[i] === null) break;
  //   var receiver_id = i;
  //   url += "?callback=PhoneScheme.jsonp_receiver[" + receiver_id + "]";
...
PhoneScheme.Dumper = PhoneScheme.Class.create({
  ...
  add_fold: function(s){
    ...
    if(lines.length > this.fold_limit){
      var fold_btn   = " <span style='text-decoration:underline; color:blue; cursor:pointer;'" +
                           "onclick='PhoneScheme.Dumper.toggle_fold("+this.n_folds+")'>more</span>";
  ...
  dump_stack: function(stk, size){
    if(stk === null || stk === undefined) return PhoneScheme.inspect(stk);
    var s = "<table>";
  ...
  dump: function(obj){
    ...
    if(obj instanceof Object){
      ...
      s = _.escape(PhoneScheme.inspect(obj)) + "<br>\n";
    }
...
PhoneScheme.Dumper.toggle_fold = function(n){
  $(".fold"+n, this.dumparea).toggle();
};
Console = {}

PhoneScheme.Port.current_error =
PhoneScheme.Port.current_output = new PhoneScheme.Port.CustomOutput(
  function (str) {
    var console;
    var text;
    console = $("#bs-console");
...
PhoneScheme.Port.current_input = new PhoneScheme.Port.CustomInput(
  function (callback) {
    var form = $("<form/>");
...
Console.puts = function(str, no_newline) {
  PhoneScheme.Port.current_output.put_string(str + (no_newline ? "" : "\n"))
};

Console.p = function (/*ARGS*/){
  PhoneScheme.Port.current_output.put_string(
    "p> "+_.map(_.toArray(arguments), PhoneScheme.inspect).join(" ")
  );
};
//
...
(function(){ //local namespace
  ...
  if ($("#biwascheme-debugger")[0]) {
    dumper = new PhoneScheme.Dumper($("#biwascheme-debugger")[0]);
  }

  // Error handler (show message to console div)
  var onError = function(e, state){
    PhoneScheme.Port.current_error.put_string(e.message + "\n");
    if (dumper) {
      dumper.dump(state);
      dumper.dump_move(1);
  ...
  var run = function(script) {
    var intp = new PhoneScheme.Interpreter(onError);
    try{
...
window['phonescheme'] = PhoneScheme;
